...
// 이거 틀렷음
var search = "ㄱㅎ";
var arr = font_test.filter((x) => {
	var cho1 = cho_hangul(x);
	var start = null;
	var cnt = 0;

	for (var i = 0; i < search.length; i++) {
		var cho = cho_hangul(search[i]);

		if (cnt > 0 || i == 0) {
			var cho2 = cho1.substring(cnt);
			var x2 = x.substring(cnt);
			var idx = (cho == '') ? cho2.indexOf(search[i]) : x2.indexOf(search[i]);
			if(i == 0) start = idx;

			if ((i == 0 && idx != -1) || (i > 0 && idx == 0)) cnt += ++idx;
			else cnt = 0;
		}
	}
	if(cnt > 0) dd(x.substring(start, cnt));
	return cnt;
})
// 이거 틀렷음
